...
public class AniPie extends FreeLine {
	 ...
	 * 饼图数据
	 */
	private List<eachPie> pieData = new ArrayList<eachPie>();
	/**
	 * 所有数据(数字)总和
	 */
	...
	/**
	 * 是否允许 滚动之后 出发选中事件 （点击事件）默认false 滚动后不触发点击扇形事件
	 */
	...
	/**
	 * 扇形从内往外 变大 默认 true false的时候 则执行 扇形扫描动画
	 */
	 ...
	 * @param defStyleAttr
	 */
	public AniPie(Context context, AttributeSet attrs, int defStyleAttr) {
	 ...
	 * @param attrs
	 */
	public AniPie(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
...
//				"构造函数-RotatablePie(Context context, AttributeSet attrs)");
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawText(pieTiele, width/2 -  measureText / 2, (height/2-wRadius-TtextSize)*1f/2-bounds.height(), titleP);
		...
		if (fillOut) {
			...
			oval = new RectF(centX - fillouting, centY - fillouting, centX
					+ fillouting, centY + fillouting);
		} else {
			// ========= 画扇形所需要的 矩形===========
			oval = new RectF(padings + mWidth / 2, padings + mHeight / 2, width
					- padings - mWidth / 2, height - padings - mHeight / 2);
		}
		...
		if (nRadius == 1) {
			nRadius = pieRadius / 2;
			Logger.d("提示线起点", "将 提示线的起点 设定到 饼图半径的一半处:" + nRadius);
		}
		...
		// ==================================饼图的展现动画===========================================
		if (!fillOut && showPieAnimation) { // 不执行fillOut动画 同时 允许出现饼图展现动画
											// 则执行扇形扫描动画
	...
	public void drawSpecialPie(Canvas canvas, float mWidth, float mHeight,
			eachPie pie, float startPie, float outORin) {
	 ...
	 * @param pieData
	 */
	public void setPiedata2(List<eachPie> pieData2) {
		// TODO
		pieData.clear();// 清空原有 数据
		pieData.addAll(pieData2);
		for (eachPie eachPie : pieData2) {
	...
	public void setPiedata(List<Float> pieData2) {
		...
		for (Float each : pieData2) {
			...
			// startAngle = ;
			eachPie pie = new eachPie("测试", each, 0, startAngle, sweepAngle);
	 ...
	 * @return
	 */
	public List<eachPie> getPieData() {
		return pieData;
	}
	...
	public class eachPie {
		...
		public eachPie(String describe, Float num, int pieColor,
	...
	public interface OnItemPieClickListener {
		 ...
		 *            被点击的扇形
		 */
		void onPieItemClick(int position, eachPie itemPie);
	}
	...
	public interface OnSpecialPieListener {
		 ...
		 *            被点击的扇形
		 */
		void onSpecialPie(int position, eachPie itemPie);
	}
