...
public class WarmLine extends View {
     ...
     * 提示线 显示 的动画失效
     */
    private boolean showCenterAll = true;
    /**
     * 刚展现
     */
    ...
    /**
     * 设置 提示线 显示在 扇形的对角线处   默认true 起始位置为内辅助圆 nRadius决定
    ...
    private float mWidth = 0;// 宽比高宽多少
    private float mHeight = 0;// 高比宽高多少
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //显示 则 分 全部显示(对角线)   点击显示(对角线，点击处) 000对角线显示(全部显示 单显示)  点击处显示(选中)
            if (anglesList.size() == 0) {
                throw new RuntimeException("数据不足请先设置数据。。。调用setPieAngles()");
            }else{
            if (showCenterAll){
                //===================================showCenterAll====展示的时候 画出全部 对角线 显示 提示线===========================================
...
//        }
    ...
    /**
     * 通过 点击的位置 计算出 提示线的 起点(Tstartx,Tstarty) 转折点(turnX,turnY) 终点(endx,turnY) 坐标
     * @param move_x
     * @param move_y
     */
    private void get3Points(float move_x, float move_y) {
        //=======================================内圆起点 经过点击点 显示 ===============================================
